...
public class PilihPemesanan extends java.awt.Dialog {
    ...
    private final String query = "SELECT pmsn.No_Pemesanan, strftime('%d - %m - %Y', pmsn.Tanggal_Pemesanan) AS Tanggal, "
            + "pmsn.ID_Supplier, spl.Nama AS Nama_Supplier, pgn.ID_Pengguna, pgn.Nama AS Nama_Pengguna, pmsn.Total_Pemesanan FROM pemesanan pmsn JOIN supplier spl "
...
//  Pengeluaran Pemesanan
    private void tampilData1(String slide) {
        try {
            PreparedStatement pst = connection.prepareStatement(query.concat("ORDER BY Tanggal_Pemesanan DESC"));
            ResultSet rst = pst.executeQuery();
            while(rst.next()) {
    ...
    private void tampilData2(String slide) {
        try {
            PreparedStatement pst = connection.prepareStatement(query.concat("WHERE Status_Pemesanan='Dikirim' ORDER BY Tanggal_Pemesanan DESC"));
            ResultSet rst = pst.executeQuery();
            while(rst.next()) {
                addRowTable(rst, tabemodel2, slide);
            }
