...
public class ServiceBarang {
    ...
    public String createKodeBarang(ModelJenisBarang modelJenisBarang) {
        ...
        String query = "SELECT substr(Kode_Barang, -3) AS Kode FROM barang WHERE Kode_Barang LIKE '"+formatJenis+"-"+format+"%' ORDER BY Kode_Barang DESC";
        try {
            PreparedStatement pst = connection.prepareStatement(query);
            ResultSet rst = pst.executeQuery();
            if(rst.next()) {
    ...
    public String createKodeJenis() {
        ...
        String query = "SELECT substr(Kode_Jenis, -3) AS Kode FROM jenis_barang ORDER BY Kode_Jenis DESC";
        try {
            PreparedStatement pst = connection.prepareStatement(query);
            ResultSet rst = pst.executeQuery();
            if(rst.next()) {
