...
public class ServiceReservasi {
    ...
    public void loadData(int page, DefaultTableModel tabmodel, Pagination pagination) {
        ...
        String query = "SELECT rsv.No_Reservasi, rsv.Tanggal_Reservasi, "
                + "rsv.ID_Pasien, psn.Nama AS Nama_Pasien, psn.Jenis_Kelamin, rsv.Tanggal_Kedatangan, "
                + "strftime('%H.%M', rsv.Jam_Kedatangan) || ' WIB' AS Jam_Kedatangan, "
                + "rsv.Status_Reservasi, rsv.ID_Pengguna, pgn.Nama AS Nama_Pengguna FROM reservasi rsv "
                + "JOIN pasien psn ON rsv.ID_Pasien=psn.ID_Pasien JOIN pengguna pgn "
                + "ON rsv.ID_Pengguna=pgn.ID_Pengguna ORDER BY No_Reservasi DESC LIMIT "+limit+" OFFSET "+(page-1) * limit+"";
    ...
    public String createNo() {
        ...
        String query = "SELECT substr(No_Reservasi, -3) AS Nomor FROM reservasi WHERE No_Reservasi LIKE 'RSV-"+strDate+"%' ORDER BY Nomor DESC";
        try {
            PreparedStatement pst = connection.prepareStatement(query);
            ResultSet rst = pst.executeQuery();
            if(rst.next()) {
