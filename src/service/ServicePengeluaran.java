...
public class ServicePengeluaran {
    ...
    public ServicePengeluaran() {
        ...
        String query = "SELECT plrn.No_Pengeluaran, plrn.ID_Pengguna, pg.Nama, plrn.Tanggal_Pengeluaran, "
                ...
                + "ORDER BY No_Pengeluaran DESC LIMIT "+limit+" OFFSET "+(page-1) * limit+"";
    ...
    public String createNo() {
        ...
        String query = "SELECT substr(No_Pengeluaran, -3) AS Nomor FROM pengeluaran WHERE No_Pengeluaran LIKE 'PLRN-"+format+"%' ORDER BY Nomor DESC";
        try {
            PreparedStatement pst = connection.prepareStatement(query);
            ResultSet rst = pst.executeQuery();
            if(rst.next()) {
