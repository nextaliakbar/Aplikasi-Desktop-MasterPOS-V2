...
public class ServiceLaporan {
    ...
    private final String queryPemeriksaan = "SELECT pmn.No_Pemeriksaan,pmn.No_Reservasi, pmn.Tanggal_Pemeriksaan, "
        ...
        + "psn.Nama AS Nama_Pasien, pmn.ID_Karyawan, krn.Nama, pmn.ID_Pengguna, pgn.Nama AS Nama_Pengguna FROM pemeriksaan pmn "
    ...
    private final String queryPenjualan = "SELECT pjl.No_Penjualan, pjl.Tanggal, "
                ...
                + "pgn.Nama AS Nama_Pengguna FROM penjualan pjl INNER JOIN pengguna pgn ON pjl.ID_Pengguna=pgn.ID_Pengguna ";
...
//  Pemeriksaan
    private void addRowTablePemeriksaan(ResultSet rst, DefaultTableModel tabmodel) throws Exception{
        ...
        String namaPasien = rst.getString("Nama_Pasien");
...
//    Pengeluaran
    ...
    public void loadAll(DefaultTableModel tabmodel, String slide) {
        ...
        String query = queryPemeriksaan.concat("WHERE strftime('%Y',Tanggal_Pemeriksaan)='"+year+"' AND strftime('%m',Tanggal_Pemeriksaan)='"+month+"' ORDER BY No_Pemeriksaan DESC");
        switch(slide) {
            case "Penjualan":
                query = queryPenjualan.concat("WHERE strftime('%Y',Tanggal)='"+year+"' AND strftime('%m',Tanggal)='"+month+"' ORDER BY No_Penjualan DESC");
                break;
            case "Pemesanan":
                query = queryPemesanan.concat("WHERE strftime('%Y',Tanggal_Pemesanan)='"+year+"' AND strftime('%m',Tanggal_Pemesanan)='"+month+"' ORDER BY No_Pemesanan DESC");
                break;
            case "Pengeluaran":
                query = queryPengeluaran.concat("WHERE strftime('%Y',Tanggal_Pengeluaran)='"+year+"' AND strftime('%m',Tanggal_Pengeluaran)='"+month+"' ORDER BY No_Pengeluaran DESC");
                break;
        }
